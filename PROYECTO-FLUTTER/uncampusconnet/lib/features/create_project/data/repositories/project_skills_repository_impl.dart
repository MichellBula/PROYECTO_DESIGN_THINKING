...
class ProjectSkillsRepositoryImpl implements ProjectSkillsRepository {
  ...
  ProjectSkillsRepositoryImpl({required this.datasource});

  @override
  Future<Map<String, dynamic>> createProjectSkill(
    ProjectSkill projectSkill,
  ) async {
    final model = ProjectSkillModel.fromEntity(projectSkill);

    return await datasource.createProjectSkill(model);
  }

  @override
  Future<List<Map<String, dynamic>>> getSkillsByProject(int idProyecto) {
    return datasource.getSkillsByProject(idProyecto);
  }
}
